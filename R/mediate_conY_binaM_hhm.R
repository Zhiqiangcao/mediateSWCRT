#' Mediation analysis function for continuous outcome and binary mediator in a stepped wedge design based on Hussey and Hughes model
#'
...
#' sigma_em = sigma_ey = 1
#' set.seed(123456)
#' mydata1 = gen_data_hhm(n,J,m,beta,gamma,betaA=0.75,betaM=1.2,gammaA=0.6,sigma_a,
...
#' res1 = mediate_contY_binaM_hhm(data=mydata1, method = "GHQ")
#' print(res1)
#' res1f = mediate_contY_binaM_hhm(data=mydata1, method = "AMLE")
#' print(res1f)
#' 
...
#' res2 = mediate_contY_binaM_hhm(data=mydata2, method = "GHQ",
#' covariateY = c("X1"), covariateM = c("X2"))
#' print(res2)
#' res2f = mediate_contY_binaM_hhm(data=mydata2, method = "AMLE",
#' covariateY = c("X1"), covariateM = c("X2"))
#' print(res2f)
#' 
#' # example 3: mediation analysis with the same covariates in both outcome and mediator models
#' res3 = mediate_contY_binaM_hhm(data=mydata2, method = "GHQ",
#' covariateY = c("X1","X2"), covariateM = c("X1","X2"))
#' print(res3)
#' res3f = mediate_contY_binaM_hhm(data=mydata2, method = "AMLE",
#' covariateY = c("X1","X2"), covariateM = c("X1","X2"))
#' print(res3f)
#' 
...
#' set.seed(1234)
#' mydata3 = gen_data_hhm(n,J,m,beta,gamma,betaA=0.75,betaM=1,gammaA=0.8,sigma_a,
#' sigma_ey,sigma_tau,sigma_em,binary.outcome=0,binary.mediator=1)
#' res4 = mediate_contY_binaM_hhm(data=mydata3,method = "GHQ")
#' print(res4)
#' res4f = mediate_contY_binaM_hhm(data=mydata3,method = "AMLE")
#' print(res4f)


mediate_contY_binaM_hhm = function(data, method = "AMLE", id = "id", outcome = "Y", mediator = "M", treatment = "A", cluster = "cluster", 
  ...
  if(method == "GHQ"){
    ...
    for(j in 1:J){
      ...
      niej = kappa_GHQ(gammaj=gammaj,gammaa=gamma_a_hat,gammax=gamma_x, sigma_tau=sigmatau)
      niej_res = beta_m_hat*(niej$trt-niej$ctrl)
      nie_j[j] = mean(niej_res)
    }
    nde_j = beta_a_hat
    te_j = nie_j+nde_j
    mp_m = mean(nie_j)/mean(te_j)  #modify on 3.28
    ...
    for(g in 1:ng){
      ...
      for(j in 1:J){
        ...
        #Gaussian-Hermite Quadrature approach
        niej_g = kappa_GHQ(gammaj=gammaj_g,gammaa=gamma_a_hat_g,gammax = gamma_x_g, sigma_tau=sigmatau_g)
        niej_g_res = beta_m_hat_g*(niej_g$trt-niej_g$ctrl)
        nie_jg[j] = mean(niej_g_res)
      }
      nde_jg = beta_a_hat_g
      te_jg = nie_jg+nde_jg
      mp_mg = mean(nie_jg)/mean(te_jg)  #have been modifed
    ...
    for(j in 1:J){
      ...
      niej = Kappa_AMLE(gammaj=gammaj,gammaa=gamma_a_hat,gammax = gamma_x, sigma_tau=sigmatau)
      niej_res = beta_m_hat*(niej$trt-niej$ctrl)
      nie_j[j] = mean(niej_res)
    }
    nde_j = beta_a_hat
    ...
    for(g in 1:ng){
      ...
      for(j in 1:J){
        ...
        if(nm == 0) xm_j = 0 else xm_j = as.matrix(xm[data_g$period==j,],ncol=nm)
        #Approximate likelihood estimation
        niej_g = Kappa_AMLE(gammaj=gammaj_g,gammaa=gamma_a_hat_g,gammax = gamma_x_g, sigma_tau=sigmatau_g)
        niej_g_res = beta_m_hat_g*(niej_g$trt-niej_g$ctrl)
        nie_jg[j] = mean(niej_g_res)
      }
      nde_jg = beta_a_hat_g
