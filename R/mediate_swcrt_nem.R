...
#' sigma_phi,sigma_ey,sigma_tau,sigma_psi,sigma_em,binary.o=0,binary.m=0)
#' res3 = mediate_swcrt_nem(data=mydata3,time.dependent = TRUE)
#' print(res3)
#' 
...
#' res4 = mediate_swcrt_nem(data = mydata4,covariate.outcome = c("X1","X2"), 
...
mediate_swcrt_nem = function(data, outcome = "Y", mediator = "M", treatment = "A", cluster = "cluster", 
  ...
  if(time.dependent == TRUE){ #exposure-time dependent treatment effect 
    if (binary.o == 0 & binary.m == 0){ #type 1: continuous outcome and continuous mediator
      ...
      delta_res = mediate_contY_binaM_nem_etm(data = data, outcome = outcome, mediator = mediator, treatment = treatment, cluster = cluster, 
                                          period = period, exposure = exposure, covariateY = covariateY, covariateM = covariateM, a0 = a0, a1 = a1)
      ...
      delta_res = mediate_binaY_binaM_nem_etm(data = data, outcome = outcome, mediator = mediator, treatment = treatment, cluster = cluster, 
                                          period = period, exposure = exposure, covariateY = covariateY, covariateM = covariateM, a0 = a0, a1 = a1)
    }
    ...
    if (binary.o == 0 & binary.m == 0){ #type 1: continuous outcome and continuous mediator
      ...
      delta_res = mediate_contY_binaM_nem(data = data, outcome = outcome, mediator = mediator, treatment = treatment, cluster = cluster, 
                                          period = period, exposure = exposure, covariateY = covariateY, covariateM = covariateM, a0 = a0, a1 = a1)
    }else if(binary.o == 1 & binary.m == 0){ #type 3: binary outcome and continuous mediator
      delta_res = mediate_binaY_contM_nem(data = data, outcome = outcome, mediator = mediator, treatment = treatment, cluster = cluster, 
                                          period = period, exposure = exposure, covariateY = covariateY, covariateM = covariateM, a0 = a0, a1 = a1)
    }else{#type 4: binary outcome and binary mediator
      delta_res = mediate_binaY_binaM_nem(data = data, outcome = outcome, mediator = mediator, treatment = treatment, cluster = cluster, 
                                          period = period, exposure = exposure, covariateY = covariateY, covariateM = covariateM, a0 = a0, a1 = a1)
    }
  }
  return(delta_res)
