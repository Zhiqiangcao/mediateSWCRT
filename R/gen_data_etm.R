#' Generate data in a stepped wedge design with an exposure-time dependent treatment effect
#' 
...
#' mydata4 = gen_data_hhm(I,J,n,beta,gamma,theta_e,beta_M=0.625,eta_e,sigma_a,
...
gen_data_etm = function(I,J,n,beta,gamma,theta_e,beta_M,eta_e,sigma_a,sigma_ey,
                        sigma_tau,sigma_em,binary.outcome=0, binary.mediator=0){
  # Generate data frame
  data <- data.frame(
  ...
  for(i in 1:I){ #I: number of cluster
    ...
    for(j in 1:J){
      ...
      if(binary.outcome == 0 & binary.mediator == 0){ #type 1: continuous outcome and continuous mediator
        m_ijk = gamma[j] + etae*A_ij + tau_i + e_ijk
        y_ijk = beta[j] + thetae*A_ij + beta_M*m_ijk + alpha_i + epsilon_ijk
      }else if(binary.outcome == 0 & binary.mediator == 1){ #type 2: continuous outcome and binary mediator
        mu_ij = gamma[j] + etae*A_ij + tau_i
        ...
        m_ijk = gamma[j] + etae*A_ij + tau_i + e_ijk
        mu_ijy = beta[j] + thetae*A_ij + beta_M*m_ijk + alpha_i
      ...
      data = rbind(data, data.frame(cbind(
        ...
        A = rep(A_ij,n), c = rep(c_i,n), alphi = rep(alpha_i,n),
