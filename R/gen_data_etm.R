#' Generate data in a stepped wedge design with a exposure time treatment effect
#' 
#' This function can generate longitudinal data in a stepped wedge design in the presence of
#' a exposure time treatment effect
#' 
...
#' @param betas the exposure effect (a J-1 vector) on the outcome conditional on mediaotr and other possible confounders
#' @param betaM the mediator effect on the outcome conditional on other possible confounders
#' @param gammas the exposure effect (a J-1 vector) on the mediator conditional other possible confounders
...
#' 
#' @references Kenny A., Voldal E.C., Xia F., Heagerty P.J. and Hughes J.P. Analysis of stepped wedge cluster randomized trials in the
...
#' sigma_em = sigma_ey = 1
#' gammas = c(0.32,0.4,0.48) 
#' betas = c(0.6,0.75,0.9)
#' # generate continuous outcome and continuous mediator
#' mydata1 = gen_data_etm(n,J,m,beta,gamma,betas,betaM=0.625,gammas,sigma_a,sigma_ey,
...
#' mydata2 = gen_data_etm(n,J,m,beta,gamma,betas,betaM=0.625,gammas,sigma_a,sigma_ey,
#' sigma_tau,sigma_em,binary.outcome=0,binary.mediator=1)
#' 
#' # generate binary outcome and continuous mediator
#' sigma_a = 0.605
#' mydata3 = gen_data_etm(n,J,m,beta,gamma,betas,betaM=0.625,gammas,sigma_a,sigma_ey,
#' sigma_tau,sigma_em,binary.outcome=1,binary.mediator=0)
#' 
#' # generate binary outcome and continuous mediator
...
#' gammas = c(0.32,0.4,0.48,0.56) 
#' betas = c(0.6,0.75,0.9,1.05)
#' sigma_tau = sigma_a = 0.334
#' sigma_em = sigma_ey = 1
#' mydata5 = gen_data_etm(n,J,m,beta,gamma,betas,betaM=0.625,gammas,sigma_a,sigma_ey,
#' sigma_tau,sigma_em,binary.outcome=0,binary.mediator=0)

gen_data_etm = function(n,J,m,beta,gamma,betas,betaM,gammas,sigma_a,sigma_ey,sigma_tau,sigma_em,
  ...
  for(i in 1:n){ #n: number of cluster
    ...
    for(j in 1:J){
      ...
      beta_s = ifelse(s > 0, betas[s], 0)
      gamma_s = ifelse(s > 0, gammas[s], 0) 
      #x1_ijk = rnorm(m)  
      ...
      if(binary.outcome == 0 & binary.mediator == 0){ #type 1: continuous outcome and continuous mediator
        m_ijk = gamma[j] + gamma_s*A_ij + tau_i + e_ijk
        y_ijk = beta[j] + beta_s*A_ij + betaM*m_ijk + alpha_i + epsilon_ijk
      }else if(binary.outcome == 0 & binary.mediator == 1){ #type 2: continuous outcome and binary mediator
        mu_ij = gamma[j] + gamma_s*A_ij + tau_i
        ...
        m_ijk = gamma[j] + gamma_s*A_ij + tau_i + e_ijk
        mu_ijy = beta[j] + beta_s*A_ij + betaM*m_ijk + alpha_i
      ...
      data <- rbind(data, data.frame(cbind(
        ...
        A = rep(A_ij,m), c = rep(c_i,m), alphi = rep(alpha_i,m),
