#' Mediation analysis function for binary outcome and binary mediator in a stepped wedge design with a time-dependent treatment effect
#'
...
#' res1 = mediate_binaY_binaM_etm(data=mydata1, method = "GHQ")
#' print(res1)
#' res1f = mediate_binaY_binaM_etm(data=mydata1, method = "AMLE")
#' print(res1f)
#' 
...
#' res2 = mediate_binaY_binaM_etm(data=mydata2, method = "GHQ",
#' covariateY = c("X1"), covariateM = c("X2"))
#' print(res2)
#' res2f = mediate_binaY_binaM_etm(data=mydata2, method = "AMLE",
#' covariateY = c("X1"), covariateM = c("X2"))
#' print(res2f)
#' 
#' # example 3: mediation analysis with the same covariates in both outcome and mediator models
#' res3 = mediate_binaY_binaM_etm(data=mydata2, method = "GHQ",
#' covariateY = c("X1","X2"), covariateM = c("X1","X2"))
#' print(res3)
#' res3f = mediate_binaY_binaM_etm(data=mydata2, method = "AMLE",
#' covariateY = c("X1","X2"), covariateM = c("X1","X2"))
#' print(res3f)
#' 
...
#' gammas = c(0.6,1.2,1.5,1.9)
#' betas = c(0.8,1.6,2,2.3)
#' set.seed(1234)
#' mydata3 = gen_data_etm(n,J,m,beta,gamma,betas,betaM=1,gammas,sigma_a,
#' sigma_ey,sigma_tau,sigma_em,binary.outcome=1,binary.mediator=1)
#' res4 = mediate_binaY_binaM_etm(data=mydata3,method = "GHQ")
#' print(res4)
#' res4f = mediate_binaY_binaM_etm(data=mydata3,method = "AMLE")
#' print(res4f)

mediate_binaY_binaM_etm = function(data, method = "AMLE", id = "id", outcome = "Y", mediator = "M", treatment = "S", 
  ...
  if(method == "GHQ"){
    ...
    for(s in 1:(J-1)){
      ...
      for(j in 1:(J-s)){
        ...
        eta_res = eta_GHQ(gammaj=gammajs[j],gammaa=gammas,gammax=gamma_x,sigma_tau=sigmatau)
        eta1 = eta_res$p1s; eta0 = eta_res$p0s
        kappa_res = kappa_GHQ(betaj=betajs[j],betaa=betas,betam=beta_m_hat,betax=beta_x,sigma_a=sigmaa)
        ...
        nie_js[j] = mean(niej) #average all i and k 
        nde_js[j] = mean(ndej) #average all i and k 
      }
    ...
    for(g in 1:ng){
      ...
      for(s in 1:(J-1)){
        ...
        for(j in 1:(J-s)){
          ...
          #Gaussian-Hermite Quadrature approach
          eta_res_g = eta_GHQ(gammaj=gammajs_g[j],gammaa=gammas_g,gammax=gamma_x_g,sigma_tau=sigmatau_g)
          eta1_g = eta_res_g$p1s; eta0_g = eta_res_g$p0s
          kappa_res_g = kappa_GHQ(betaj=betajs_g[j],betaa=betas_g,betam=beta_m_hat_g,betax=beta_x_g,sigma_a=sigmaa_g)
          ...
          nie_js_g[j] = mean(niej_g) #average all i and k 
          nde_js_g[j] = mean(ndej_g) #average all i and k 
        }
        nie_s_g[s] = mean(nie_js_g) #average all j
        nde_s_g[s] = mean(nde_js_g) #average all j
      }
    ...
    for(s in 1:(J-1)){
      ...
      for(j in 1:(J-s)){
        if(ny == 0) xy_j = 0 else xy_j = as.matrix(xy[data$period==(j+s),ncol=ny])
        if(nm == 0) xm_j = 0 else xm_j = as.matrix(xm[data$period==(j+s),ncol=nm])
        #approximate likelihood estimation approach
        approx_res = cal_prob_AMLE(gammaj=gammajs[j],gamma=gammas,gammax=gamma_x,sigma_tau=sigmatau,
                                   betaj=betajs[j],betaa=betas,betam=beta_m_hat,betax=beta_x,sigma_a=sigmaa)
    ...
    for(g in 1:ng){
      ...
      for(s in 1:(J-1)){
        ...
        for(j in 1:(J-s)){
          if(ny == 0) xy_j = 0 else xy_j = as.matrix(xy[data_g$period==(j+s),ncol=ny])
          if(nm == 0) xm_j = 0 else xm_j = as.matrix(xm[data_g$period==(j+s),ncol=nm])
          approx_res_g = cal_prob_AMLE(gammaj=gammajs_g[j],gamma=gammas_g,gammax=gamma_x_g,sigma_tau=sigmatau_g,
                                       betaj=betajs_g[j],betaa=betas_g,betam=beta_m_hat_g,betax=beta_x_g,sigma_a=sigmaa_g)
