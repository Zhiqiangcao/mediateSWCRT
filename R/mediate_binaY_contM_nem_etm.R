...
mediate_binaY_contM_nem_etm = function(data, outcome = "Y", mediator = "M", treatment = "A", cluster = "cluster", 
  ...
  cal_prob = function(betaj,theta,betam,betax,gammaj,eta,gammax,sigma_em,sigma_tau,sigma_a,sigma_phi,sigma_psi){
    q_11 = exp(betaj+theta*a1+betam*(gammaj+eta*a1+as.numeric(xm_j%*%gammax))+as.numeric(xy_j%*%betax))/(1+
    ...
    #
    q_10 = exp(betaj+theta*a1+betam*(gammaj+eta*a0+as.numeric(xm_j%*%gammax))+as.numeric(xy_j%*%betax))/(1+
    ...
    #
    q_00 = exp(betaj+theta*a0+betam*(gammaj+eta*a0+as.numeric(xm_j%*%gammax))+as.numeric(xy_j%*%betax))/(1+
  ...
  for(e in 1:(El-1)){
    ...
    for(j in 1:(J-e-dje)){
      ...
      pr10 = prob_res$pr_10
      pr00 = prob_res$pr_00
      niej_res = log(pr11/(1-pr11))-log(pr10/(1-pr10))
      ndej_res = log(pr10/(1-pr10))-log(pr00/(1-pr00))
  ...
  for(g in 1:ng){
    ...
    for(e in 1:(El-1)){
      ...
      for(j in 1:(J-e-dje)){
        ...
        niej_res_g = log(pr11_g/(1-pr11_g))-log(pr10_g/(1-pr10_g))
        ndej_res_g = log(pr10_g/(1-pr10_g))-log(pr00_g/(1-pr00_g))
