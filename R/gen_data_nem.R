#' Generate data in a stepped wedge design based on nested exchangeable correlation model with a constant treatment effect
#' 
...
gen_data_nem = function(I,J,n,beta,gamma,theta,beta_M,eta,sigma_a,sigma_phi,sigma_ey,
  ...
  data = data.frame(
    ...
    "alpha" = numeric(), # random effects of outcome within a cluster
  ...
  for(i in 1:I){ #I: number of cluster
    ...
    for(j in 1:J){
      ...
      if(binary.o == 0 & binary.m == 0){ #type 1: continuous outcome and continuous mediator
        m_ijk = gamma[j] + eta*A_ij + tau_i + psi_ij + e_ijk
        y_ijk = beta[j] + theta*A_ij + beta_M*m_ijk + alpha_i + phi_ij + epsilon_ijk
      }else if(binary.o == 0 & binary.m == 1){ #type 2: continuous outcome and binary mediator
        mu_ij = gamma[j] + eta*A_ij + tau_i + psi_ij
        ...
        m_ijk = gamma[j] + eta*A_ij + tau_i + psi_ij + e_ijk
        mu_ijy = beta[j] + theta*A_ij + beta_M*m_ijk + alpha_i + phi_ij
      ...
      data = rbind(data, data.frame(cbind(
        ...
        A = rep(A_ij,n), c = rep(c_i,n), alpha = rep(alpha_i,n), phi = rep(phi_ij,n),
