...
#' res1 = mediate_binaY_binaM_nem_etm(data=mydata1)
#' print(res1)
#' 
...
#' res2 = mediate_binaY_binaM_nem_etm(data = mydata2, covariateY = c("X1"),
#' covariateM = c("X2"))
#' print(res2)
#' 
#' # example 3: mediation analysis with the same covariates in outcome and mediator models
#' res3 = mediate_binaY_binaM_nem_etm(data = mydata2,
...
#' set.seed(123456)
#' mydata3 = gen_data_nem_etm(I,J,n,beta,gamma,theta_e,beta_M=1,eta_e,sigma_a,
#' sigma_phi,sigma_ey,sigma_tau,sigma_psi,sigma_em,binary.o=1,binary.m=1)
#' res4 = mediate_binaY_binaM_nem_etm(data=mydata3)
#' print(res4)
#' 


mediate_binaY_binaM_nem_etm = function(data, outcome = "Y", mediator = "M", treatment = "A", cluster = "cluster", 
  ...
  Kappa_STA = function(gammaj,eta,gammax,sigma_tau,sigma_psi){
    ...
    res = data.frame(p1s = kappa_1j, p0s = kappa_0j)
    return(res)
  }
  
  ##compute double integral \kappa (double STA method)
  lambda_STA = function(betaj,theta,betam,betax,sigma_a,sigma_phi){
    ...
    C1_11 = (1+0.5*sigma_a^2)*(mu11+(mu11-3*mu11^2+2*mu11^3)*0.5*sigma_phi^2)
    C2_11 = -3/2*sigma_a^2*(mu11^2+(4*mu11^2-10*mu11^3+6*mu11^4)*0.5*sigma_phi^2)
    C3_11 = sigma_a^2*(mu11^3+(9*mu11^3-21*mu11^4+12*mu11^5)*0.5*sigma_phi^2)
    lambda11 = C1_11+C2_11+C3_11
    
    C1_10 = (1+0.5*sigma_a^2)*(mu10+(mu10-3*mu10^2+2*mu10^3)*0.5*sigma_phi^2)
    C2_10 = -3/2*sigma_a^2*(mu10^2+(4*mu10^2-10*mu10^3+6*mu10^4)*0.5*sigma_phi^2)
    C3_10 = sigma_a^2*(mu10^3+(9*mu10^3-21*mu10^4+12*mu10^5)*0.5*sigma_phi^2)
    lambda10 = C1_10+C2_10+C3_10
    
    C1_01 = (1+0.5*sigma_a^2)*(mu01+(mu01-3*mu01^2+2*mu01^3)*0.5*sigma_phi^2)
    C2_01 = -3/2*sigma_a^2*(mu01^2+(4*mu01^2-10*mu01^3+6*mu01^4)*0.5*sigma_phi^2)
    C3_01 = sigma_a^2*(mu01^3+(9*mu01^3-21*mu01^4+12*mu01^5)*0.5*sigma_phi^2)
    lambda01 = C1_01+C2_01+C3_01
    
    C1_00 = (1+0.5*sigma_a^2)*(mu00+(mu00-3*mu00^2+2*mu00^3)*0.5*sigma_phi^2)
    C2_00 = -3/2*sigma_a^2*(mu00^2+(4*mu00^2-10*mu00^3+6*mu00^4)*0.5*sigma_phi^2)
    C3_00 = sigma_a^2*(mu00^3+(9*mu00^3-21*mu00^4+12*mu00^5)*0.5*sigma_phi^2)
    lambda00 = C1_00+C2_00+C3_00
    
    res = data.frame(p11s = lambda11, p10s = lambda10, p01s = lambda01, p00s = lambda00)
    return(res)
  }
  ...
  dje = J-El  #if there is no buffer period, then J=El and dje=0
  #number of clusters
  ng = length(clusters)
  ...
  for(e in 1:(El-1)){
    ...
    gammaje = gamma_est[(e+1+dje):J]
    nie_je = nde_je = numeric(J-e-dje)
    for(j in 1:(J-e-dje)){
      if(ny == 0) xy_j = 0 else xy_j = as.matrix(xy[data$period==(j+e+dje),],ncol=ny)
      if(nm == 0) xm_j = 0 else xm_j = as.matrix(xm[data$period==(j+e+dje),],ncol=nm)
      kappa_res = Kappa_STA(gammaj=gammaje[j],eta=etae,gammax=gamma_x,sigma_tau=sigmatau,sigma_psi=sigmapsi)
      kappa1 = kappa_res$p1s; kappa0 = kappa_res$p0s
      lambda_res = lambda_STA(betaj=betaje[j],theta=thetae,betam=beta_m_hat,betax=beta_x,sigma_a=sigmaa,sigma_phi = sigmaphi)
  ...
  for(g in 1:ng){
    ...
    for(e in 1:(El-1)){
      ...
      for(j in 1:(J-e-dje)){
        if(ny == 0) xy_j = 0 else xy_j = as.matrix(xy[data_g$period==(j+e+dje),],ncol=ny)
        if(nm == 0) xm_j = 0 else xm_j = as.matrix(xm[data_g$period==(j+e+dje),],ncol=nm)
        #Gaussian-Hermite Quadrature approach
        kappa_res_g = Kappa_STA(gammaj=gammaje_g[j],eta=etae_g,gammax=gamma_x_g,sigma_tau=sigmatau_g,sigma_psi=sigmapsi_g)
        kappa1_g = kappa_res_g$p1s; kappa0_g = kappa_res_g$p0s
        lambda_res_g = lambda_STA(betaj=betaje_g[j],theta=thetae_g,betam=beta_m_hat_g,betax=beta_x_g,sigma_a=sigmaa_g,sigma_phi = sigmaphi_g)
