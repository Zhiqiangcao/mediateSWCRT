...
#' set.seed(123456)
#' mydata1 = gen_data_nem(I,J,n,beta,gamma,theta=1,beta_M=1,eta=1,sigma_a,
...
#' res1 = mediate_binaY_binaM_nem(data=mydata1)
#' print(res1)
#' 
...
#' res2 = mediate_binaY_binaM_nem(data = mydata2, covariateY = c("X1"), 
#' covariateM = c("X2"))
#' print(res2)
#' 
#' # example 3: mediation analysis with the same covariates in outcome and mediator models
#' res3 = mediate_binaY_binaM_nem(data = mydata2, covariateY = c("X1","X2"), 
#' covariateM = c("X1","X2"))
#' print(res3)
#' 
...
#' set.seed(123456)
#' mydata3 = gen_data_nem(I,J,n,beta,gamma,theta=1,beta_M=1,eta=1,sigma_a,
#' sigma_phi,sigma_ey,sigma_tau,sigma_psi,sigma_em,binary.o=1,binary.m=1)
#' res4 = mediate_binaY_binaM_nem(data=mydata3)
#' print(res4)
#' 

mediate_binaY_binaM_nem = function(data, outcome = "Y", mediator = "M", treatment = "A", cluster = "cluster", 
  ...
  for(j in 1:J){
    ...
    #double STA approximation
    kappa_res = Kappa_STA(gammaj=gammaj,eta=eta_hat,gammax=gamma_x,sigma_tau=sigmatau,sigma_psi=sigmapsi)
    kappa1 = kappa_res$p1s; kappa0 = kappa_res$p0s
    lambda_res = lambda_STA(betaj=betaj,theta=theta_hat,betam=beta_m_hat,betax=beta_x,sigma_a=sigmaa,sigma_phi = sigmaphi)
  ...
  for(g in 1:ng){
    ...
    if(ny == 0){
      ...
      beta_x_g = model_Y_g$coefficients[-(1:(J+2)),1] #in case the above phenomenon occurs
    ...
    for(j in 1:J){
      ...
      kappa_res_g = Kappa_STA(gammaj=gammaj_g,eta=eta_hat_g,gammax=gamma_x_g,sigma_tau=sigmatau_g,sigma_psi=sigmapsi_g)
      kappa1_g = kappa_res_g$p1s; kappa0_g = kappa_res_g$p0s
      lambda_res_g = lambda_STA(betaj=betaj_g,theta=theta_hat_g,betam=beta_m_hat_g,betax=beta_x_g,sigma_a=sigmaa_g,sigma_phi = sigmaphi_g)
